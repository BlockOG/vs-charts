...
function diffChanged() {
    ...
    // chart_image.src = `/vs-charts/hidden/charts/${chart}/${difficulty_names[difficulty]}-${scroll_speed.toFixed(1)}.png`;
    chart_image.src = `/vs-charts/hidden/charts/${chart}/${difficulty_names[difficulty]}.png`;
}
...
if (url.searchParams.has("chart")) {
    ...
    fetch("/vs-charts/hidden/song_data.json").then((data) => {
        data.json().then((data) => {
